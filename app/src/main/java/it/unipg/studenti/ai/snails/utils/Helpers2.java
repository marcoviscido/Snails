...
public class Helpers2 {
    ...
    public static Mat[] findROI(Mat src) {
        ...
        Mat binaryImg = new Mat(src.size(), CvType.CV_8UC1); //muore nella findContours
        ...
        //USO I CONTORNI APPENA TROVATI PER CREARE UNA MASCHERA DELL'IMMAGINE
        Mat
                = new Mat(src.size(), CvType.CV_8UC1, new Scalar(255,255,255));
        Imgproc.drawContours(mask, contours, idx, new Scalar(255,255,255),1,Imgproc.LINE_AA,new Mat(), 0,new Point(0,0));
        Imgproc.floodFill(mask, new Mat(), new Point(2,2) , new Scalar(0,0,0) );

        //src.copyTo(maskedImage,mask); // creates masked Image and copies it to maskedImage

        RotatedRect rr = Imgproc.minAreaRect(new MatOfPoint2f(contours.get(idx).toArray()));
        Rect delimiter = rr.boundingRect();

        Mat croppedSrc = new Mat();
        src.copyTo(croppedSrc);


        //Imgproc.drawContours(mask, contours, idx, new Scalar(255,255,0),3);
        //Imgproc.rectangle(mask, delimiter.tl(), delimiter.br(), new Scalar(255,255,255),5,Imgproc.LINE_AA,0);

        //Imgproc.rectangle(src, delimiter.tl(), delimiter.br(), new Scalar(255,255,255),5);
        //maskedImage = src.submat(delimiter);
